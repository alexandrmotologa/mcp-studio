...
- 🔍 **Auto-Discovery:** Instant 1-click import of MCP servers configured in Claude Desktop (`claude_desktop_config.json`) and Cursor IDE (`mcp.json`).
- ⚡️ **Dynamic Tool Runner:** Automatically generates interactive visual forms from tool JSON schemas + raw JSON mode with execution timers and output inspectors.
- 🗄️ **Resource Inspector:** Explore and read live resources (`file://`, `postgres://`, `api://`) with MIME-type formatting.
- 💬 **Prompt Template Tester:** Fill in template parameters and render prompt messages in real-time.
- 🧠 **AI Agent Simulator (Pro):** Multi-LLM test bench supporting Anthropic Claude 3.7/3.5, OpenAI GPT-4o/o3-mini, Google Gemini 2.0, and local Ollama (Llama 3.2, DeepSeek-R1) to watch your agent select and execute tools live.
- 🛰️ **Live JSON-RPC 2.0 Traffic Stream:** Real-time stream of incoming/outgoing payloads, latency benchmarks, and error diagnostics.
- 📤 **1-Click IDE Export:** Export formatted configurations ready for Cursor, Claude Desktop, and Windsurf.
